...
type AssetCurrencyBarCardProps = {
  title: string;
  data: CompareAssetItem[];
  colors?: string[];
};

const AssetCompareBarCard = (props: AssetCurrencyBarCardProps) => {
  const { data, title, colors } = props;
  const currencyMap = useCurrencyStore(selectCurrencyMap);

  const _colors = useMemo(() => {
    const defaultColors = ['#3B82F6', '#F59E0B'];
    if (Array.isArray(colors) && colors.length >= 2) return colors;
    if (Array.isArray(colors) && colors.length === 1) return [colors[0], defaultColors[1]];
      ...
      <CardContent sx={{ height: '100%' }}>
        ...
        <ResponsiveContainer width="100%" height="85%">
          <BarChart data={data} margin={{ top: 10, right: 10, left: 0, bottom: 0 }} barGap={6}>
            ...
            />
            <Bar dataKey="current" name="current" fill={_colors[0]} radius={[6, 6, 0, 0]} />
            <Bar dataKey="previous" name="previous" fill={_colors[1]} radius={[6, 6, 0, 0]} />
          </BarChart>
        </ResponsiveContainer>
      </CardContent>
    </Card>
...
export default AssetCompareBarCard;
